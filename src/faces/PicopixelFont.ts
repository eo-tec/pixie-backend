// Picopixel bitmap font — exact copy from Adafruit GFX Fonts/Picopixel.h
// Each glyph: { width, height, xAdvance, xOffset, yOffset, bitmap }
// bitmap is packed MSB-first, same as Adafruit GFX
...
export interface GlyphData {
  ...
  xOffset: number;
  yOffset: number;
  bitmap: number[]; // packed bits, MSB-first
}

const PICOPIXEL_BITMAPS: number[] = [
...
// Glyph descriptors: [bitmapOffset, width, height, xAdvance, xOffset, yOffset]
const PICOPIXEL_GLYPHS: [number, number, number, number, number, number][] = [
  [0, 0, 0, 2, 0, 1],     // 0x20 ' '
  [0, 1, 5, 2, 0, -4],    // 0x21 '!'
  [1, 3, 2, 4, 0, -4],    // 0x22 '"'
  [2, 5, 5, 6, 0, -4],    // 0x23 '#'
...
export function getGlyph(char: string): GlyphData | null {
  const code = char.charCodeAt(0);
  if (code < FIRST_CHAR || code > LAST_CHAR) return null;
  const idx = code - FIRST_CHAR;
  const [offset, width, height, xAdvance, xOffset, yOffset] = PICOPIXEL_GLYPHS[idx];

  if (width === 0 || height === 0) {
    return { width: 0, height: 0, xAdvance, xOffset, yOffset, bitmap: [] };
  }

  // Calculate how many bytes this glyph uses
  const bits = width * height;
  const bytes = Math.ceil(bits / 8);
  const bitmap = PICOPIXEL_BITMAPS.slice(offset, offset + bytes);

  return { width, height, xAdvance, xOffset, yOffset, bitmap };
}
...
export function measureText(text: string): { width: number; height: number } {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (!g) {
      w += 2; // space for unknown chars
      continue;
    }
    if (i < text.length - 1) {
      w += g.xAdvance;
    } else {
      // Last character: use actual width
      w += g.xOffset + g.width;
    }
    minY = Math.min(minY, g.yOffset);
    maxY = Math.max(maxY, g.yOffset + g.height);
  }
  return { width: w, height: maxY - minY };
}
